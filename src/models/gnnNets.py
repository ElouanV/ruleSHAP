...
class GCNNet(GNNBase):
    ...
    def get_emb(self, rule_masks=None,  replaced_emb=None,*args, **kwargs):
        ...
        for i in range(self.num_gnn_layers):
            ...
            if rule_masks is not None:
                rule_mask=  rule_masks[i].unsqueeze(0)
                x = x * rule_mask
            if replaced_emb is not None:
                # Replace non zero elements in replaced_emb of the layer in x
                for j in range(x.shape[0]):
                    x[j] = torch.where(replaced_emb[i] != 0, replaced_emb[i], x[j])
        return x

    def forward(self, deactivated_rules=None, replaced_emb=None, *args, **kwargs):
        _, _, batch = self._argsparse(*args, **kwargs)
        # node embedding for GNN
        emb = self.get_emb(rule_masks=deactivated_rules, replaced_emb=replaced_emb,*args, **kwargs)
        # pooling process
